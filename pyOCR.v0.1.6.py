...
# 窗口置顶与取消
def fix_window():
    global root, btnText_WindowFixTop
    if(btnText_WindowFixTop.get()=="置顶"):
        root.wm_attributes("-topmost", True)
        btnText_WindowFixTop.set("取消")
    else:
        root.wm_attributes("-topmost", False)
        btnText_WindowFixTop.set("置顶")
...
# 程序信息
def show_about():
    about = msg.showinfo("关于", "pyOCR v0.1.6(2023.6.17)\n\n作者:\t张皖豫(1738296705@qq.com)\n技术支持:\t百度开放平台\n函数库:\ttkinter, PIL, time, ctypes, pyautogui, pyperclip, requests, base64, hashlib, json, os")
...
if(__name__=='__main__'):
    ...
    """ 定义一些组件的样式 """
    style = Style() 
    style.configure("NML.TButton",  #noraml button
            background="#faffe6",
            foreground="#000000",
            width=4)
    style.configure("CLS.TButton",  # clean button
            background="#faffe6",
            foreground="#ff0000",
            width=4)
    style.configure("ABT.TButton",  # about button
            background="#ccffff",
            foreground="#003c65",
            width=4)
    ...
    """ 全屏截图并识别 """
    btn_ScreenAllShot = Button(frame1,text="截图",command=ScreenShootAllGraph,style='NML.TButton')
    ...
    """ 翻译 """
    global btnText_TextTrans
    btnText_TextTrans = StringVar()
    btnText_TextTrans.set("翻译")
    btn_TextTrans = Button(frame1,textvariable=btnText_TextTrans,command=trans_text_Box,style='NML.TButton')
    ...
    """ 窗口置顶与取消 """
    global btnText_WindowFixTop
    btnText_WindowFixTop = StringVar()
    btnText_WindowFixTop.set("置顶")
    btn_WindowFixTop = Button(frame1,textvariable=btnText_WindowFixTop,command=fix_window,style='NML.TButton')
    btn_WindowFixTop.pack(side='left')

    """ 清空文本框 """
    btn_TextCls = Button(frame1,text="清空",command=clear_text_Box,style='CLS.TButton')
